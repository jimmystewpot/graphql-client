...
const SCHEMA_JSON: &str = include_str!("atlassian_schema.json");
const SCHEMA_GRAPHQL: &str = include_str!("atlassian_schema.graphql");
...
fn ast_from_graphql_and_json_produce_the_same_schema() {
    ...
    // Input objects
    {
        ...
        for (json, gql) in json.stored_inputs.iter().zip(gql.stored_inputs.iter()) {
            assert_eq!(json.is_one_of, gql.is_one_of, "isOneOf doesn't match for {}", json.name);
            assert_eq!(json.fields.len(), gql.fields.len());
        }
    }
